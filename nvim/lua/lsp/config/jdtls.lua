...
-- 查找启动器 jar - 使用平台特定的启动器
local launcher_pattern
if is_windows then
    launcher_pattern = jdtls_server_path .. path_sep .. "plugins" .. path_sep .. "org.eclipse.equinox.launcher.win32.win32.x86_64_*.jar"
else
    launcher_pattern = jdtls_server_path .. path_sep .. "plugins" .. path_sep .. "org.eclipse.equinox.launcher_*.jar"
end

local launcher_jar = vim.fn.glob(launcher_pattern)
if launcher_jar == "" then
    -- 尝试使用正斜杠作为备选
    local alt_pattern = launcher_pattern:gsub("\\", "/")
    launcher_jar = vim.fn.glob(alt_pattern)
    if launcher_jar == "" then
        vim.notify(
            "找不到 jdtls 启动器 jar，请检查路径。模式: "
            .. launcher_pattern,
